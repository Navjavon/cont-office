<template>
  <div id="inspire">
      ...
      <v-card-text class="pl-6 pr-6 pt-4 pb-0">
        <v-simple-table>
          <template v-slot:default>
            ...
            <tr v-for="(item, index) in filteredGroups" :key="index">
              <td>{{index + 1}}</td>
              <td>{{item.name}}</td>
              <td>
    ...
    <add-edit-modal v-if="showAddEdit" :addEditData="addEditData" @on-close-add-modal="onCloseAddModal"></add-edit-modal>
    <confirm-modal :confirmData="confirmData" @on-close-confirm="onCloseConfirm"/>
  </div>
</template>

<script lang="ts" src="./ComplaintsReason.ts"/>
