...
export default class SuggestionsGroupComponent extends Vue {
  ...
  private findGroup(value: string) {
    ...
    this.filteredGroups = this.groups.filter(({name}) => name.toLowerCase().includes(value));
  }

  private async getGroups() {
    this.groups = this.filteredGroups = await GroupService.getSuggestionGroups();
  }
}
